...
- Default to one authorized source Git commit per completed, verified Bead and one branch/PR per coherent change, which may contain several related Beads. Beads track execution, commits checkpoint implementation, and PRs define review/merge boundaries; they need not be one-to-one. Do not create a branch/PR just because a Bead finishes, or an empty commit for a no-change review. See the workflow guide for validation checkpoints and scoped approval.
...
- `dolt.auto-commit` is explicitly `on`: ordinary `bd` writes manage local Dolt history transparently, without a separate approval or routine `bd dolt commit` step. Git commits and all remote synchronization still require explicit approval. Do not enable auto-push, auto-backup publication, or automatic staging.
...
- Build: `uv build` produces sdist/wheel in `dist/`. Linux installed-package smoke: `uv run --locked python tests/installed_package_smoke.py --temp-root /tmp` (existing directory outside checkout); builds wheel from sdist, installs hashed locked runtime dependencies, and runs isolated help/fixture ETL with packaged migrations and independent values. Requires package/Spatial access or caches; no optimized Python assertions.
- CI: `.github/workflows/ci.yml` runs PR-to-main/main-push Python 3.13 jobs, `Linux quality and wheel` (full non-fixing quality/coverage plus installed smoke) and `Windows runtime and publication` (ten targeted cases, no full-suite coverage gate). Both explicitly provision/check Spatial and fail on errors; workflow presence does not configure required branch checks.
...
- Persisted verification compares committed counts with accepted loader inputs and checks every geometry as a nonempty point with finite longitude `[-180,180]` and latitude `[-90,90]`, inclusive. It does not repair coordinates (`270E` stays `270.0` and fails acceptance) or impose nonzero counts; it is not a whole-source audit.
- The CLI builds and verifies a unique sibling candidate, closes it, then atomically publishes it. Existing output requires `--replace` and stays untouched until publication; quiesce external writers (destination WAL is refused). Processing failures clean only run-owned artifacts; publication failures retain the verified candidate at the reported path for manual recovery. Post-publication cleanup/summary failures warn without failing the run. Use ordinary local filesystems; replacement does not retain a backup.
- Follow [README recovery and limits](README.md#failure-and-manual-recovery): no forced clobber/copy fallback or old-WAL deletion. One run per destination, enough space for old output plus candidate/transient state, and operator backups when needed. No network-filesystem, hostile-path, concurrent-writer or power-loss/crash-recovery guarantee; unexplained crash leftovers are not verified candidates.
- The CLI initializes candidate schema with packaged Alembic migrations (`initialize_database(connection)`) before loading and owns disposal of both load and summary engines. UoW entry does not create or repair tables; callers must initialize schema explicitly. `alembic.ini` and the default UoW target `data/hurdat.duckdb`, while the CLI injects its run-owned candidate. The optional summary uses a separate read-only session on the published output.
- CLI entry owns one structlog JSON-line handler for `logs/pipeline.log` relative to the working directory, bridging stdlib stage/library logs with run ID, operation/stage and error context. Rich remains console-only. Run exit closes the handler and restores caller logging level/context; constructing a stage does not configure logging or create files. Tests isolate their working directories so normal test runs do not touch the developer's log.
- Existing host handlers/library thresholds remain intact. Logs append without automatic rotation or a concurrent-run/audit guarantee. Full records and SQL bound parameters are avoided by default, but paths/IDs/exception details can appear; review before sharing. Execute manual CLI validation only in isolated working directories to avoid the developer's log.
